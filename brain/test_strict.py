...
diag_vertical_strict = """@startuml Arquitectura_Capas_FashionStore
skinparam monochrome true
skinparam shadowing false
skinparam packageStyle rectangle

package "UI - Frontend (Web & Móvil)\\n<<capa de presentación>>" as Layer_UI {
    package "Autenticación y Perfil - UI" as UI_Auth
    package "Catálogo, Carrito y POS - UI" as UI_Commerce
    package "Probador AR y Asistente IA - UI" as UI_AR
}
...
package "BD & Entorno\\n<<capa de persistencia e infraestructura>>" as Layer_Infra {
    ...
    package "Servicios Cloud\\n(Stripe / SIN / Azure Blob)" as Ext_Services
    package "Docker Compose\\n(Orquestación y Despliegue)" as Infra_Docker
}

' Forzar apilado vertical estricto entre capas
Layer_UI -[hidden]down-> Layer_App
Layer_App -[hidden]down-> Layer_Domain
Layer_Domain -[hidden]down-> Layer_Infra
...
Srv_Auth ..> Dom_Cat
Srv_Commerce ..> Dom_Cat
Srv_Commerce ..> Dom_Inv
Srv_Commerce ..> Dom_Res
Srv_Commerce ..> Dom_Pay
...
url = plantuml_url(diag_vertical_strict)
req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
try:
    resp = urllib.request.urlopen(req)
    with open("c:/Parcial-si2/brain/arch_strict.png", "wb") as f:
        f.write(resp.read())
    print("SUCCESS: Strict vertical diagram saved!")
